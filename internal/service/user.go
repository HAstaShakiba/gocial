...
var (
	...
	// ErrUserNotFound used when the user wasn't found on th db.
...
func (s *Service) ToggleFollow(ctx context.Context, username string) (ToggleFollowOutput, error) {
	...
	if err != nil {
		return out, fmt.Errorf("could not begin tx:v $v", err)
	}
	defer tx.Rollback()
	...
	if out.Following {
		...
		if _, err = tx.ExecContext(ctx, query, followerID); err != nil {
			return out, fmt.Errorf("could not update follower followess count (-): %v", err)
		}
